    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex flex-column flex-sm-row flex-md-row align-items-md-center justify-content-between">
                <div>
                    <h4 class="card-title text-center text-lg-start text-uppercase mb-2 mb-md-0 mb-lg-0">Kategori Proker</h4>
                </div>
                <div class="text-center">
                    <a href="{{ route('app.work_program_category.create') }}" class="btn btn-primary">Tambah</a>
                </div>
            </div>
        </div>
        <div class="card-body">
                    ...
                    <tbody>
                        @foreach ($work_program_categories as $wp_category)
                            <tr>
                                ...
                                <td>{{ $wp_category->slug }}</td>
                                <td>{{ $wp_category->percentage }}%</td>
                                <td>
                                    <div class="d-flex gap-2">
                                        <div class="edit">
                                            <a href="{{ route('app.work_program_category.edit', $wp_category->id) }}"
                                                class="btn btn-sm btn-success">Edit</a>
                                        </div>
                                        <div class="remove">
                                            <form action="{{ route('app.work_program_category.destroy', $wp_category->id) }}"
                                                method="post">
                                                @csrf
                                                @method('delete')
