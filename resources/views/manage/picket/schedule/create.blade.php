            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h4 class="card-title text-center text-uppercase mb-2 mb-md-0 mb-lg-0">Tambah Jadwal Piket</h4>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <form action="{{ route('app.picket_schedule.store') }}" method="post">
                        @csrf
                        <div class="row align-items-center mb-3">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control" name="day" id="day">
                                    ...
                                    @foreach ($days as $day)
                                        <option value="{{ $day->id }}" {{ select_old($day->id, old('day')) }}>
                                            {{ $day->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control select2" name="users[]" id="users" data-placeholder="Pilih Staff/Guru" multiple>
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}"
                                            {{ select_old_multiple($user->id, old('users')) }}>
                                            {{ $user->name }}
                                        </option>
                                    @endforeach
                                </select>
