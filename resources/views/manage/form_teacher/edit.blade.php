            ...
            <div class="card">
                <div class="card-header">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h4 class="card-title text-center text-uppercase mb-2 mb-md-0 mb-lg-0">Edit Wali Kelas</h4>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <form action="{{ route('app.form_teacher.update', $form_teacher->id) }}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="row align-items-center mb-3">
                            ...
                            <div class="col-sm-9">
                                <span>{{ $form_teacher->user->name }}</span>
                            </div>
                        </div>
                        <div class="row align-items-center mb-3">
                            <div class="col-sm-3">
                                ...
                                <select class="form-control" data-choices data-choices-search-false name="classroom"
                                    ...
                                    @foreach ($classrooms as $classroom)
                                        <option value="{{ $classroom->id }}"
                                            {{ select_old($classroom->id, old('classroom'), true, $form_teacher->classroom_id) }}>
                                            {{ $classroom->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control" data-choices data-choices-search-false
                                    ...
                                    @foreach ($expertises as $expertise)
                                        <option value="{{ $expertise->id }}"
                                            {{ select_old($expertise->id, old('expertise'), true, $form_teacher->expertise_id) }}>
